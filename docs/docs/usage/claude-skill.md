...
3. Regenerate the shared AI instructions with `n8nac update-ai` so the project root contains `AGENTS.md`.
4. Tell any planner/coder handoff file (for example `CLAUDE.md`) to read `AGENTS.md` and to use `n8nac skills` before writing workflow logic.
...
# 3) Refresh the shared agent instructions
