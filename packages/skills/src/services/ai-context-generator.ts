...
export class AiContextGenerator {
  ...
  private getSharedResponseFormatLines(cliCmd: string): string[] {
    return [
      ...
      `11. For webhook/chat/form workflows: run \`${cliCmd} test <id>\` after pushing.`,
      ...
      `---`,
      ``,
      `Remember: Check initialization first. Pull before you modify. Push after you modify. Test webhook/chat/form workflows after push. Never guess parameters — always verify against the schema.`,
    ];
  }
  ...
  private getAgentsContent(n8nVersion: string, distTag?: string): string {
    ...
    return [
      ...
      ``,
      `6. **TEST (recommended for webhook/chat/form workflows)**: Execute the workflow in test mode`,
      ...
      ``,
      `7. **RESOLVE CONFLICTS**: If Push or Pull fails due to a conflict`,
      ...
      `- **test after push for webhook/chat/form workflows**: Run \`${cliCmd} test <id>\` to close the dev cycle. A Class A error is not a bug — tell the user. A Class B error is fixable — iterate.`,
      ``,
      ...
      ``,
      `### Step 6: Test Webhook/Chat/Form Workflows After Push`,
      `\`\`\`bash`,
      ...
      `\`\`\``,
      `Closes the dev cycle for webhook/chat/form workflows. Exits 0 on success or Class A (config gap — inform user). Exits 1 on Class B (wiring error — fix and re-test).`,
      ``,
      `---`,
      ``,
