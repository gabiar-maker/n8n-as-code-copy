...
export class AiContextGenerator {
  ...
  private getSharedResponseFormatLines(cliCmd: string): string[] {
    return [
      ...
      `5. For a new workflow, read \`workflowDir\` from the active instance in \`n8nac-config.json\` to find the correct directory, create the file there, and confirm it appears in \`${cliCmd} list --local\` before pushing.`,
  ...
  private getAgentsContent(
    ...
    return [
      ...
      `   - Read \`n8nac-config.json\` to understand the active sync context. The config defines \`syncFolder\`, \`instanceIdentifier\`, \`projectName\`, and the pre-computed \`workflowDir\` (the full relative path where workflow files live). \`n8nac\` builds the full local path from this; you never need to reconstruct it manually.`,
      ...
      `   - For a brand-new workflow: read \`workflowDir\` from the **active instance** in \`n8nac-config.json\` — that is the instance whose \`id\` matches \`activeInstanceId\`. That relative path is the canonical location for all workflow files. Create the file there; never in the workspace root.`,
      ...
      `   - After writing a new file, confirm it appears in \`${cliCmd} list --local\` before running \`${cliCmd} push <relative-path>\` with the full relative path such as \`workflows/127_0_0_1_5678_yagr_l/personal/slack-notification.workflow.ts\`.`,
      ``,
      `4. **PUSH**: Upload your changes explicitly`,
      `   - \`${cliCmd} push <relative-path>\`: Upload the local workflow file to n8n. This is the only public push form.`,
      `   - \`${cliCmd} push <relative-path> --verify\`: Push and immediately verify the live workflow against the local schema.`,
      ``,
      `   > ⚠️ **CRITICAL — what \`relative-path\` means**:`,
      `   > - Always use the **full relative path from the workspace root** including the \`.workflow.ts\` suffix.`,
      `   > - The path must start with \`workflowDir\` (from the active instance in \`n8nac-config.json\`), for example \`workflows/127_0_0_1_5678_yagr_l/personal/slack-notification.workflow.ts\`.`,
      ...
      `- **new workflows must be created in the active local workflow directory**: Read \`workflowDir\` from the active instance in \`n8nac-config.json\` (the instance whose \`id\` === \`activeInstanceId\`) — this is always correct regardless of \`--instance\` overrides or prior \`instance select\` calls. Do not write workflows in the repo root or an ad-hoc folder.`,
      `- **push requires the full relative path from the workspace root**: Always use \`workflowDir/<filename>.workflow.ts\` — never a bare filename. A bare name has no implicit scope prefix and will be rejected with a clear error showing the expected path.`,
      ...
      ``,
      `> \`pull\` and \`resolve\` always operate on **a single workflow ID**. \`push\` always starts from **the full relative path of the local workflow file** (e.g. \`workflows/127_0_0_1_5678_yagr_l/personal/my-workflow.workflow.ts\`). \`list\` is the only command that covers all workflows at once.`,
      ``,
