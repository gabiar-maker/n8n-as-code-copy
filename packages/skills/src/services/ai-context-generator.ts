...
`;
  ...
  private getAgentsContent(n8nVersion: string): string {
    return [
      ...
      ``,
      `n8n-as-code uses a strict GitOps architecture. The local code is the source of truth, but the user might have modified the workflow in the n8n UI.`,
      ``,
      `**⚠️ CRITICAL RULE**: Before modifying ANY existing \`.workflow.ts\` file, you MUST ensure you have the latest remote changes.`,
      ``,
      `1. **PULL FIRST**: Always run \`n8n-as-code pull <workflow-id>\` (or use the equivalent tool) before reading or editing a workflow file.`,
      `2. **READ**: Read the updated local file.`,
      `3. **EDIT**: Apply your changes.`,
      `4. **SAVE**: Saving the file will automatically push it to n8n.`,
      ``,
