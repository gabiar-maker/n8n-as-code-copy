...
export class AiContextGenerator {
  ...
  private getWorkspaceBootstrapLines(cliCmd: string): string[] {
    return [
      ...
      `- If the user has already provided the n8n host and API key, run \`${cliCmd} init --yes --host <url> --api-key <key>\` immediately.`,
      `- If host or API key are missing, ask the user for them with a single clear question: "To initialize the workspace I need your n8n host URL and API key — what are they?" Then, once you have both values, run \`${cliCmd} init --yes --host <url> --api-key <key>\` yourself.`,
      `- Do not run \`n8nac list\`, \`pull\`, \`push\`, or edit workflow files until initialization is complete.`,
      `- Never write \`n8nac-config.json\` by hand. Initialization must go through \`${cliCmd} init --yes\` so credentials and AI context stay consistent.`,
      ...
      `2. If missing: check if \`N8N_HOST\` and \`N8N_API_KEY\` are set in the environment — if so, run \`${cliCmd} init --yes\` directly.`,
      `3. If missing and env vars are absent: ask the user for the host URL and API key, then run \`${cliCmd} init --yes --host <url> --api-key <key>\` yourself. **Do not ask the user to run the command.**`,
